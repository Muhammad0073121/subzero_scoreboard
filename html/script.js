  ...
  for (const i in table.players) {
    if (table.useidentity) {
      table.players[i].name =
        "" + table.players[i].firstname + " " + table.players[i].lastname + "";
    }
    ...
    if (table.players[i].ping > 300) {
      ping =
        ...
        table.players[i].ping +
        "";
    } else if (table.players[i].ping > 100) {
      ping =
        '<i style="color:yellow;" class="fa-solid fa-wifi"></i> ' +
        table.players[i].ping +
        "";
    }
    var divid =
      `<span class="leaderboard__name" style="float: left;position: absolute;left: -1.5vw;bottom: 0%;font-weight: 700;font-size: 14px;width: auto;min-width: 55px;background: #0808087d;border-radius: 5px;color: #daebf5;"><i class="fa-solid fa-id-card"></i> ` +
      table.players[i].id +
      `</span>`;
    if (!showid && !admin) {
      divid = "";
    ...
    if (table.players[i].admin) {
      admindiv = `<span class="leaderboard__name admin" style="float: left;position: absolute;left: 3vw;font-weight: 700;font-size: 15px;bottom: 8%;color: gold;"><i class="fa-solid fa-crown"></i></span>`;
    }
    if (!showadmins) {
      admindiv = "";
    }
    ...
    if (table.players[i].vip) {
      vipdiv = `<span class="leaderboard__name vip" style="float: left;position: absolute;left: 30px;font-weight: 700;font-size: 15px;bottom: 22px;color: #d206d2;"><i class="fa-solid fa-star"></i></span>`;
    }
    if (!showvip) {
      vipdiv = "";
    }
    ...
    if (table.usediscordname) {
      discdiv =
        '<span class="leaderboard__name discordname" style="float: left;position: absolute;left: 3vw;font-weight: 700;font-size: 1vh;bottom: 0.5vh;"><i class="fa-brands fa-discord"></i> ' +
        table.players[i].discordname +
        "</span>";
    }
...
document.onkeyup = function (data) {
  if (data.keyCode == "27") {
    // Escape key 76 = L (Change the 76 to whatever keycodes you want to hide the carlock ui LINK https://css-tricks.com/snippets/javascript/javascript-keycodes/)
    $.post("https://subzero_scoreboard/close", {}, function (datab) {});
    document.getElementById("scoreboard").style.display = "none";
    document.getElementById("main").innerHTML = "";
    document.getElementById("jobs").innerHTML = "";
  }
    ...
    for (const i in $("form").serializeArray()) {
      ...
      if (checkURL(url[i].value)) {
        if (imageExists(url[i].value)) {
          console.log("UPLOAD");
          $.post(
            "https://subzero_scoreboard/avatarupload",
            JSON.stringify({ url: url[i].value }),
            function (datab) {}
          );
        }
      }
