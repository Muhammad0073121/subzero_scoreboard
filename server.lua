...
QBCore.Functions.CreateCallback('subzero_scoreboard:playerlist', function(source, cb)
    ...
    for k,v in pairs(players) do
        ...
        if Player ~= nil then
            ...
            table.insert(list, {id = v.id, job = Player.PlayerData.job.name, name = v.name, discordname = v.discordname, firstname = v.first, lastname = v.last, image = v.image, ping = ping, admin = isAdmin(v.id), vip = v.vip, gang = Player.PlayerData.gang.name})
        end
    end
    local count = 0
