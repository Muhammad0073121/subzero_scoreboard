...
CreateThread(function()
    ...
    TriggerClientEvent("subzero_scoreboard:loaded", -1)
end)
...
function UploadAvatar(citizenid, avatar)
    ...
    exports.ghmattimysql:execute('UPDATE players SET metadata=@metadata WHERE citizenid=@citizenid',
        { ['@metadata'] = json.encode(Player.PlayerData.metadata), ['@citizenid'] = citizenid })
end

RegisterNetEvent('subzero_scoreboard:avatarupload')
AddEventHandler('subzero_scoreboard:avatarupload', function(url)
...
function GetAvatar(source, first, last)
    ...
    if steamhex ~= nil and steamhex ~= '' then
        ...
        GetConvar('steam_webApiKey') .. '&steamids=' .. steamid, function(e, data, h)
            local data = json.decode(data)
            local avatar = data.response.players[1].avatarfull
            image = avatar
        end)
        local c = 0
        while image == nil and c < 100 do
            c = c + 1
            Wait(1)
        end
        if image == nil then image = 'https://ui-avatars.com/api/?name=' ..
            first .. '+' .. last .. '&background=' .. letters.background .. '&color=' .. letters.color .. '' end
        return image
    else
        return 'https://ui-avatars.com/api/?name=' ..
        first .. '+' .. last .. '&background=' .. letters.background .. '&color=' .. letters.color .. ''
    end
end

local loading = {}
RegisterNetEvent('subzero_scoreboard:playerloaded')
AddEventHandler('subzero_scoreboard:playerloaded', function()
    local source = tonumber(source)
    local Player = QBCore.Functions.GetPlayer(source)
    local initials = math.random(1, #config.RandomAvatars)
    local letters = config.RandomAvatars[initials]
    if players[source] == nil and Player ~= nil and loading[source] == nil then
        ...
        if config.UseSelfUploadAvatar then
            if Player.PlayerData.metadata['avatar'] ~= nil and Player.PlayerData.metadata['avatar'] ~= '' then
                ...
                f .. '+' .. l .. '&background=' .. letters.background .. '&color=' .. letters.color .. ''
            end
        ...
        if players[source] == nil then
            players[source] = { id = source, image = avatar, first = f, last = l, name = name, discordname =
            GetDiscordName(source, f, l), vip = v }
        end
    end
end)
...
QBCore.Functions.CreateCallback('subzero_scoreboard:playerlist', function(source, cb)
    local list = {}
    ...
    for k, v in pairs(players) do
        ...
        for _, gang in pairs(config.gangjobs) do
            local gang = gang.name
            if gangjobs[gang] == nil then
                gangjobs[gang] = 0
            end
            if Player ~= nil and Player.PlayerData.gang.name == gang then
                gangjobs[gang] = gangjobs[gang] + 1
                break
            end
        end
        if Player ~= nil then
            ...
            table.insert(list,
                { id = v.id, job = Player.PlayerData.job.name, name = v.name, discordname = v.discordname, firstname = v
                .first, lastname = v.last, image = v.image, ping = ping, admin = isAdmin(v.id), vip = v.vip, gang =
    ...
    if Player ~= nil then
        cb(list, whitelistedjobs, count, isAdmin(source), players[source].image, gangjobs)
    end
end)
